...
//and cookiemains for playtesting this mod, you can look at the cookiemains section to see what ideas he suggested
		...
		decay.setRates = function() {
			...
			d *= Math.pow(0.9985, Math.log10(c));
			...
			d *= Math.pow(0.998, Math.max(Math.pow(decay.getBuildingContribution(), 0.33) - 10, 0));
		...
		Game.registerHook('check', () => {
			if (Game.Objects['Wizard tower'].minigameLoaded && !grimoireUpdated) {
				...
				if (typeof gp.spells === 'undefined') { return false; }
				var M = gp;
				decay.addSpells();
		...
		Game.registerHook('check', () => {
			if (Game.Objects['Temple'].minigameLoaded && !pantheonUpdated) {
				...
				eval('pp.logic='+pp.logic.toString().replace('t=1000*60*60*16', 't=1000*5*60*16').replace('t=1000*60*60*16', 't=1000*80*60').replace('t=1000*60*60*4', 't=1000*60*20'));
