...
//and cookiemains for playtesting this mod, you can look at the cookiemains section to see what ideas he suggested
		...
		Game.veilParticleSizeMax = function(set) {
			return 96 * Math.pow(0.5, set) * Math.pow((Game.veilHP / Game.veilMaxHP), 0.6 + set * 0.1);
		}
		Game.veilParticleSpeed = function(set) {
			return 96 * Math.pow(0.6, set) * Math.pow(Game.veilHP / Game.veilMaxHP, 0.6 + set * 0.15);
		}
		...
		veilParticles = veilParticles.replace('var xx=x+Math.sin(a)*(110+r*16);', 'var xx=x+Math.sin(a)*(110+r*Game.veilParticleSpeed(set));').replace('var yy=y+Math.cos(a)*(110+r*16);', 'var yy=y+Math.cos(a)*(110+r*Game.veilParticleSpeed(set));');
